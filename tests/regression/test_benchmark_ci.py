"""Regression: benchmark AUROC values must not drift from the v0.7.1 baseline.

JSON key structure (v0.7.1):
    graphsage, gat                  - inductive GNNs (primary benchmark)
                                      GraphSAGE: 0.9714 [0.9625, 0.9797]
                                      GAT:       0.9690 [0.9590, 0.9778]
    node2vec_inductive              - topology-only, train walks only (0.9890)
    node2vec_transductive           - full-graph walks; supplementary only (0.9965)
    quantum_kernel, classical_rbf   - node classification; supplementary only
    structure_structure_of_protein  - secondary task (both GNNs: 0.9971)
"""
import json
...
def test_gat_ci_method_is_bootstrap():
    """v0.6.0 uses 1000x bootstrap CIs, not mann_whitney_se."""
    data = _load()
