...
| Ground truth source | Peer-reviewed papers and clinical trial records (2016-2025) |
...
|---|-----------------|------------------|-----------------------|------------|--------|-----------|
| 1 | Sickle cell, BCL11A enhancer | SpCas9 | ~80% HDR in HSC | #1 | Yes | Frangoul 2021 *NEJM* |
...
The failures are not random. Each one reveals a specific boundary of the
current rule-based scoring heuristic.
...
>200x higher activity in human cells). The heuristic does not distinguish
...
These three cases mark where a structure-aware, activity-aware scoring model
would improve on the current rule-based heuristic.
...
mechanism preference (DSB-free over DSB) and delivery fit. The table below
shows this design choice explicitly for the 7 successful scenarios.
...
feasibility over raw activity. SpCas9 scores low (0.42) despite 95% knockout
efficiency because it induces DSBs. This is by design: ATLAS is a
decision-support tool, not an efficiency predictor.
...
- **Efficiency data is sparse.** Many systems report only qualitative success.
  No regression model is fitted; the efficiency column above is descriptive.
...
| Item | Target | Status |
|------|--------|--------|
| Expand to 50+ scenarios | Broader mechanistic and disease coverage | Planned |
| Quantitative efficiency regression | Requires N > 500 locus-level data points (currently ~80 in literature) | Blocked on data |
| Prospective validation | Design scenario, run ATLAS, run wet-lab experiment | Requires wet-lab collaboration |
| Clinical trial correlation | Link ATLAS rank to IND-stage editors | Longer-term |
