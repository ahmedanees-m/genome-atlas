...
def node2vec_probe(G, emb_map: dict, edge_type_key: str, seed: int = 42):
    ...
    Negatives are sampled from the *correct node types* (src_type x dst_type)
...
def main():
    ...
    print("Loading Node2Vec data...")
    n2v_df = pd.read_parquet(args.node2vec)
    # Detect format: old = per-node embeddings (node_id + embedding columns);
    # new = inductive metrics summary (edge_type + auroc_mean columns).
    _n2v_is_metrics = "edge_type" in n2v_df.columns and "auroc_mean" in n2v_df.columns
    if _n2v_is_metrics:
        print(f"  Detected inductive-metrics format ({len(n2v_df)} rows) - "
              "will import Node2Vec CIs directly instead of re-probing.")
        emb_map = {}   # unused in this branch
    else:
        emb_map = dict(zip(n2v_df["node_id"], n2v_df["embedding"].apply(np.array)))
        print(f"  Loaded per-node embeddings for {len(emb_map):,} nodes.")
    ...
    for method, preds_path in [("GraphSAGE", args.graphsage_preds), ("GAT", args.gat_preds)]:
        ...
        for et, grp in df.groupby("edge_type"):
            ...
            results.append({
                ...
                "n_test": len(y_true),
                # Evaluation mode metadata: distinguishes inductive from transductive
                "evaluation_mode": "Inductive",
                "notes": "Test edges withheld from message-passing during GNN training",
            })

    # ------------------------------------------------------------------ #
    # Node2Vec: probe per edge type (or import pre-computed inductive CIs) #
    # ------------------------------------------------------------------ #
    print("\n  Node2Vec...")
    if _n2v_is_metrics:
        # New format: inductive metrics already computed by node2vec_inductive_v6.py
        for _, row in n2v_df.iterrows():
            results.append({
                "method":          "Node2Vec",
                "edge_type":       row["edge_type"],
                "auroc_mean":      row["auroc_mean"],
                "auroc_lo":        row.get("auroc_lo"),
                "auroc_hi":        row.get("auroc_hi"),
                "auprc_mean":      row.get("auprc_mean"),
                "auprc_lo":        row.get("auprc_lo"),
                "auprc_hi":        row.get("auprc_hi"),
                "n_test":          row.get("n_test"),
                "evaluation_mode": row.get("evaluation_mode", "Inductive"),
                "notes":           row.get("notes", "Imported from node2vec_inductive_v6"),
            })
            print(f"    {row['edge_type']}: AUROC={row['auroc_mean']:.4f} "
                  f"[{row.get('auroc_lo', float('nan')):.4f}, "
                  f"{row.get('auroc_hi', float('nan')):.4f}]  (imported)")
    else:
        # Old format: per-node embeddings -> transductive probe
        ...
        for et in sorted(edge_types_in_graph):
            ...
            results.append({
                ...
                # full graph including test edges. This inflates AUROC.
