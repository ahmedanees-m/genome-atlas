...
* :mod:`genome_atlas.api` - Public query API (``Atlas`` class)
* :mod:`genome_atlas.selection` - Therapeutic editor selection engine
* :mod:`genome_atlas.models.graphsage` - Heterogeneous GNN models (SAGEConv / GATConv)
* :mod:`genome_atlas.graph.build` - PyG graph builder (``build_pyg_hetero``, ``add_train_val_test_split``)
* :mod:`genome_atlas.graph.view` - Graph view API (``get_graph(graph_view='primary'|'full')``)
